      ...
      <el-col :span="7">
        <el-card shadow="hover">
          <el-form-item label="任务名称" prop="taskname">
            <el-input placeholder="请输入任务名称" maxlength="10" v-model="form.taskname" :disabled="true"></el-input>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      sceneList: [],
      props: { multiple: true },
      sceneMap: new Map([["home", "TPR"], ["office", "TPR"], ["market", "TPR"], ["outdoors", "TPR"], ["car_80km", "TPR"], ["car_80km_r", "TPR"], ["AEC_8", "AEC"], ["AEC_16", "AEC"], ["IAR", "IAR"], ["FA", "FA"]]),
      options: [{
        value: 'TPR',
        label: 'TPR',
        children: [
    ...
    clickSave(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.form.modelList = this.form.modelList.join(',')
          this.form.projectList = projectList.join(',')
          this.form.sceneList = sceneList.join(',')
          this.form.wordList = this.form.wordList.join(',')
          this.$api.task.editTask(this.form).then(({ data }) => {
            var json = JSON.parse(data);
            if (json.status == 0) {
              this.$api.voice.editTaskVoice({
  ...
  mounted() {
    ...
    this.form = this.$store.state.task.curTask
    this.form.sceneList = this.form.sceneList.split(',')
    this.form.psList = new Array(this.form.sceneList.length)
    this.form.sceneList.forEach((item, index) => {
      this.form.psList[index] = new Array(2)
      this.form.psList[index][0] = this.sceneMap.get(item)
      this.form.psList[index][1] = item
    })

    this.$api.voice.getTaskVoice({ task: this.form.taskname }).then(({ data }) => {
      var json = JSON.parse(data);
      if (json.status == 0) {
        this.rightVoiceData = json.result
        this.rightVoiceCount = `已加入语料（${this.rightVoiceData.length}）`
        this.bubbleSort(this.rightVoiceData)
      }
    })
    ...
    this.$api.file.getTrainFile().then(({ data }) => {
      ...
      if (json.status == 0) {
        json.result.forEach(item => {
          if (this.rightVoiceData.findIndex(p => p.voice == item.voice) == -1) {
            this.remainVoiceData.push(item)
          }
        })
        this.leftVoiceData = JSON.parse(JSON.stringify(this.remainVoiceData))
